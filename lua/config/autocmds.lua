...
local function get_sass_config()
  ...
  local prompt = default_file ~= "" and string.format("Target Sass file (default: %s): ", default_file)
    or "Target Sass file: "
  ...
  -- 出力ディレクトリの設定
  output_dir = vim.fn.input(string.format("Output directory (default: %s): ", output_dir), output_dir)
  if output_dir == "" then
...
vim.api.nvim_set_keymap(
  ...
  "<leader>csq",
  ":SassStop<CR>",
  { noremap = true, silent = true, desc = "Stop sass watch" }
)
